...
printf(
	...
	esc_html__( 'Amount: %s', 'beltoft-gift-cards-for-woocommerce-pro' ),
	esc_html( wp_strip_all_tags( wc_price( $gift_card->initial_amount, array( 'currency' => $gift_card->currency ) ) ) )
);
echo "\n";

printf(
	/* translators: %s: gift card code */
	esc_html__( 'Code: %s', 'beltoft-gift-cards-for-woocommerce-pro' ),
...
printf(
	...
	esc_html__( 'Shop now: %s', 'beltoft-gift-cards-for-woocommerce-pro' ),
	esc_url( add_query_arg( 'bgcw_apply', rawurlencode( $gift_card->code ), wc_get_page_permalink( 'shop' ) ) )
);
echo "\n\n";

esc_html_e( 'Click the link above to apply your gift card automatically, or enter the code at checkout in the coupon/gift card field.', 'beltoft-gift-cards-for-woocommerce-pro' );
echo "\n\n";
