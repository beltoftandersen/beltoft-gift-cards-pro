...
class Generator {
	...
	public static function init() {
		add_action( 'wp_ajax_wcgc_pro_bulk_generate', [ __CLASS__, 'ajax_bulk_generate' ] );
		add_action( 'wp_ajax_wcgc_pro_bulk_progress', [ __CLASS__, 'ajax_bulk_progress' ] );
	}
	...
	public static function ajax_bulk_generate() {
		...
		if ( $quantity < 1 || $quantity > 500 ) {
			wp_send_json_error(
				[ 'message' => __( 'Quantity must be between 1 and 500.', 'smart-gift-cards-for-woocommerce-pro' ) ]
			);
		}
		...
		if ( $amount <= 0 ) {
			wp_send_json_error(
				[ 'message' => __( 'Amount must be greater than zero.', 'smart-gift-cards-for-woocommerce-pro' ) ]
			);
		}

		// Store progress transients for polling.
		set_transient( 'wcgc_pro_bulk_total', $quantity, HOUR_IN_SECONDS );
		...
		if ( $has_prefix ) {
			...
			\GiftCards\Support\Options::invalidate_cache();
		}
		...
		for ( $i = 0; $i < $quantity; $i++ ) {
			...
			if ( $gc_id ) {
				TransactionRepository::insert( [
					...
					'note'          => __( 'Bulk generated by admin', 'smart-gift-cards-for-woocommerce-pro' ),
				] );
			...
			set_transient( 'wcgc_pro_bulk_progress', $created, HOUR_IN_SECONDS );
		}
		...
		// Clean up transients.
		delete_transient( 'wcgc_pro_bulk_total' );
		delete_transient( 'wcgc_pro_bulk_progress' );

		wp_send_json_success( [
			'created' => $created,
			/* translators: %d: number of gift cards generated */
			'message' => sprintf( __( '%d gift card(s) generated successfully.', 'smart-gift-cards-for-woocommerce-pro' ), $created ),
		] );
	}
	...
	public static function ajax_bulk_progress() {
		...
		$total    = (int) get_transient( 'wcgc_pro_bulk_total' );
		$progress = (int) get_transient( 'wcgc_pro_bulk_progress' );
