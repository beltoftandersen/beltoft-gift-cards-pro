...
class CartHandler {
	...
	/** @var array<int,bool> Prevent re-entrant processing per order in the same request. */
	private static $in_progress_orders = [];
	...
	public static function check_bogo( $gc_id, $order ) {
		...
		if ( isset( self::$in_progress_orders[ $order_id ] ) ) {
			return;
		}
		...
		self::$in_progress_orders[ $order_id ] = true;
